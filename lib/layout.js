...
define(function () {
  ...
  function fillNodes(node) {
	...
	if(node.pos)
	{
		if(!isNaN(node.pos[0]))	node.style.left = node.pos[0];
		if(!isNaN(node.pos[1]))	node.style.top = node.pos[1];
	}
	
	if (node.margin[0] == node.margin[1] == node.margin[2] == node.margin[3]) node.style.margin = node.margin[0];
	if (node.flexwrap) node.style.flexWrap = node.flexwrap;
	
	if (node.sizetocontent)
	{
		node.style.measure = node.sizetocontent.bind(node);
	}
	node.style.marginLeft = node.margin[0];
	node.style.marginRight = node.margin[2];
	node.style.marginTop = node.margin[1];
	node.style.marginBottom = node.margin[3];

	
	
	if (node.padding[0] == node.padding[1] == node.padding[2] == node.padding[3]) node.style.padding = node.padding[0];
	node.style.paddingLeft = node.padding[0];
	node.style.paddingRight = node.padding[2];
	node.style.paddingTop = node.padding[1];
	node.style.paddingBottom = node.padding[3];
	
	if (node.flexdirection) node.style.flexDirection = node.flexdirection;
	if (node.size){
		if (!isNaN(node.size[0]))	node.style.width = node.size[0];
		if (!isNaN(node.size[1]))	node.style.height = node.size[1];
	}
	if (node.position) node.style.position = node.position;
	if (node.flex) node.style.flex = node.flex;
