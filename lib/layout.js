...
define(function () {
	...
	function getSpacing(node, type, suffix, locations) {
		for (var i = 0; i < locations.length; ++i) {
			...
			var key = '_' + type + capitalizeFirst(location) + suffix;
			if (key in node) {
				var prop = node[key]
				if (prop && prop.struct) return prop[0]
				return prop
			}
	...
	function getPositiveSpacing(node, type, suffix, locations) {
		for (var i = 0; i < locations.length; ++i) {
			...
			var key = '_' + type + capitalizeFirst(location) + suffix;
			if (key in node && node[key] >= 0) {
				if (node[key].struct) return node[key][0];
				return node[key];
			}
	...
	function isUndefined(value, name) {
		...
		var lengthzero = (value === "" );
		var valundef = value === undefined ;
		var valnull = value === null;
		var isNaNVal = (typeof(value) === "number" && isNaN(value));

		//if (name === "alignitems")
		//{
		//	console.log(value, lengthzero, valundef, valnull, isNaNVal);
		//}

		return (lengthzero || valundef || valnull || isNaNVal)		;
	}
	...
	function getJustifyContent(node) {
		if ('_justifycontent' in node) {
			return node._justifycontent;
		}
		return 'flex-start';
	}

	function getAlignContent(node) {
		if ('_aligncontent' in node) {
			return node._aligncontent;
		}
		return 'flex-start';
	}

	function getAlignItem(node, child) {
		if (isUndefined(child[ '_alignself' ] )=== false) {
			//console.log(" alignself " + child.alignself );
			return child._alignself;
		}
		if (isUndefined(node['_alignitems' ],"alignitems" ) === false) {
		//	console.log(" alignitems " + node.alignitems );
			return node._alignitems;
		}
		return 'stretch';
	}
	...
	function isDimDefined(node, axis) {
		var dimname = '_'+dim[axis]
		return !isUndefined(node[dimname]) && node[dimname] >= 0;
	}

	function isPosDefined(node, pos) {
		return !isUndefined(node[pos]);
	}

	function isMeasureDefined(node) {
		return 'measure' in node;
	}
	...
	function boundAxis(node, axis, value) {
		var min = {
			'row' : node._minwidth,
			'row-reverse' : node._minwidth,
			'column' : node._minheight,
			'column-reverse' : node._minheight
		}
		[axis];

		var max = {
			'row' : node._maxwidth,
			'row-reverse' : node._maxwidth,
			'column' : node._maxheight,
			'column-reverse' : node._maxheight
		}
		[axis];
	...
	function setDimensionFromStyle(node, axis) {
		...
		node.layout[dim[axis]] = fmaxf(
				boundAxis(node, axis, node[dim[axis]]),
	...
	function getRelativePosition(node, axis) {
		var axisname = '_' + leading[axis]
		if (axisname in node) {
			return getPosition(node, '_' + leading[axis]);
		}
		return -getPosition(node, '_' + trailing[axis]);
	}
	...
	function layoutNode(node, parentMaxWidth, /*css_direction_t*/
		...
		if (isMeasureDefined(node)) {
			...
			var /*bool*/
			isRowUndefined = !isDimDefined(node, resolvedRowAxis) &&
				isUndefined(node.layout[dim[resolvedRowAxis]]);
			var /*bool*/
			isColumnUndefined = !isDimDefined(node, CSS_FLEX_DIRECTION_COLUMN) &&
				isUndefined(node.layout[dim[CSS_FLEX_DIRECTION_COLUMN]]);

			// Let's not measure the text if we already know both dimensions
			if (isRowUndefined || isColumnUndefined) {
				var /*css_dim_t*/
				measureDim = node.measure(
						/*(c)!node->context,*/
		...
		while (endLine < node.children.length) {
			...
			for (i = startLine; i < endLine; ++i) {
				...
				if (getPositionType(child) === CSS_POSITION_ABSOLUTE &&
					...
					var /*float*/
					leadingCrossDim = getLeadingPaddingAndBorder(node, crossAxis);
		...
		if (linesCount > 1 &&
			...
			crossDimLead = 0;
			var /*float*/
			currentLead = getLeadingPaddingAndBorder(node, crossAxis);
