...
define(function () {
	...
	function fillNodes(node) {
		...
		if (node.alignitems) node.style.alignItems = node._alignitems;
		if (node.alignself) node.style.alignSelf = node._alignself;
		if (node.flexwrap) node.style.flexWrap = node._flexwrap;
		if (node.sizetocontent) node.style.measure = node.sizetocontent.bind(node);
		if (node.flexdirection) node.style.flexDirection = node._flexdirection;
		...
		if (node.position) node.style.position = node._position;
		if (node.flex) node.style.flex = node._flex;
