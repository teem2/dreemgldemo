define.browserClass(function(require, view){
	...
	this.bg.fractzoom = function(pos, time, zoom){
		...
		var kali1 =  demos.kali2d(mypos + vec2(0.0001*time), 30, vec2(-0.8280193310201044,-0.858019331020104-dx))
	...
	this.bg.highdefblirpy = function(pos, time, zoom){
		var xs = 20. * zoom
		var ys = 22. * zoom
	...
	this.touchmove = function(){
		var x = this.touch.x / this.screen.size[0]
		this.bg.x1 = x
	}

	this.bg.bgcolorfn = function(pos, tex){
		return highdefblirpy(mesh.xy, time * 4., 1.0)
		//return fractzoom(mesh.xy, time, 10.)
	}
})